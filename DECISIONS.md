...
## A16 — the plan's boundary allowlist is stale against A9/A10/A15 — deviation

**Question:** Task 5's `allowed_re` is copied from the plan, but run against the repo it governs it rejects five files. Widen it, and to what?
**Options considered:** copy the plan's regex verbatim and let the gate fail / widen until the gate is green / widen only to files traceable to a recorded authorisation / escalate
**Chosen:** Widen to exactly seven additions, each with a traceable warrant: `.nvmrc` and `TESTING.md` (fork-owned new files created by Task 1, which the plan's list simply omits), and `scripts/generate-blog.mjs` (A9), `docs/.vitepress/config.mts` (A10), `Dockerfile` (A15) — three upstream files the operator authorised after the plan was written.
**Decided-by:** claude
**Justification:** The plan's regex predates A9, A10 and A15, so a verbatim copy is red on arrival — it rejects work the operator explicitly approved. The failure mode to avoid is the opposite one: widening until the gate passes would make it ratify whatever drift already exists, which is precisely what AC-01 exists to prevent. Each addition was therefore checked against `git cat-file -e "$base:$f"` to classify it as fork-owned-new versus upstream-modified, and every upstream-modified entry traced to its authorising decision before being admitted. `scripts/generate-blog.mjs` is 67 changed lines and so was added to `allowed_re` rather than `capped`, because A9 widened the allowance without imposing a line cap; putting it under the 25-line rule would retroactively invalidate an approved change. Assumed rather than escalated: reversible, determinable from the decision ledger, and it changes no shipped artefact. Verified by three negative controls on the live repo (unrelated upstream file → exit 1 naming it; capped file at 44 lines → `44 > 25`; an upstream dependency version bump → `dependencies.@fontsource/cedarville-cursive changed/removed`), each returning clean after restore.
**Outcome:** applied
**Ref:** `scripts/check-upstream-boundary.sh`; supersedes the plan's Task 5 Step 2 regex

## A17 — `npm run lint` ordering: the gate must precede eslint — gate-resolution

**Question:** AC-01 names `npm run lint` as the boundary gate, but the plan's `eslint . && bash scripts/check-upstream-boundary.sh` never reaches the gate, because eslint exits 1 on the pre-existing `public/sw.js` error recorded in A14.
**Options considered:** run the boundary check first / a separate `lint:boundary` script with both run under `;` / silence the upstream eslint error so `&&` works as the plan assumed
**Chosen:** `"lint": "bash scripts/check-upstream-boundary.sh && eslint ."` — the operator selected this option directly.
**Decided-by:** human
**Justification:** Measured, not inferred: with the plan's ordering, `npm run lint` exits 1 and `grep -c 'boundary:'` over its output is **0** — the gate is dead code, so AC-01 would be nominally satisfied and actually unenforced, the worst of the three outcomes. `public/sw.js` is unmodified from `UPSTREAM_BASE` (`git diff --numstat` against the base is empty), so its error is upstream's, arriving with a lint dependency update per A14; silencing it would mean editing upstream lint config to satisfy a fork gate — plausibly an AC-01 breach by the very rule being enforced. Reordering changes no exit code in the passing case (eslint's pre-existing 1 still governs) while guaranteeing the gate always runs and can fail the command on its own. Verified both directions: clean tree → `boundary: clean against 597e3690…` then eslint's `1 error, 2620 warnings`, exit 1; injected breach → gate names `README.md`, eslint never runs (`grep -c 'problems'` is 0), exit 1.
**Outcome:** applied
**Ref:** `package.json` `scripts.lint`

## A18 — `docker-compose.yml` is an upstream file, not a new one — gate-resolution

**Question:** Task 6 says "create `docker-compose.yml`", but the file already exists in `UPSTREAM_BASE` — it is BentoPDF's own compose, pointing at `ghcr.io/alam00000/bentopdf-simple:latest`. It also sits in the boundary gate's **uncapped** `allowed_re`, so replacing it wholesale passes the gate silently. Replace it, or take a fork-specific filename?
**Options considered:** replace it and record the warrant / leave upstream's file untouched and add `docker-compose.toolhub.yml` / replace it silently as the plan's wording implies
**Chosen:** Replace the file, keep it allowlisted, and record this entry as its warrant. The operator selected this option directly.
**Decided-by:** human
**Justification:** Found by auditing every `allowed_re` entry against `git cat-file -e "$base:$f"`, not just the entries A16 added: exactly four are upstream-owned, and three (`scripts/generate-blog.mjs` A9, `docs/.vitepress/config.mts` A10, `Dockerfile` A15) carry recorded warrants. `docker-compose.yml` was the only one without — inherited from the plan's draft regex and never justified, which is the same stale-allowlist defect A16 recorded, missed on the first pass because A16 audited the additions rather than the inherited list. The fork genuinely needs its own compose (different image tag, container name, port bindings and healthcheck), and upstream's would be actively wrong to ship, so a fork-specific filename would leave a misleading file in the tree and force every `docker compose` call to carry `-f`. The residual risk this entry accepts: an allowlisted upstream file can be rewritten without the gate objecting, so the ledger rather than the gate is what records the intent. **The healthcheck was measured, not copied:** the plan offered `curl` with a `wget` fallback, and `docker run --rm --entrypoint sh quay.io/nginx/nginx-unprivileged:alpine-slim -c 'command -v curl || command -v wget'` returns no curl and `/usr/bin/wget`, so the `wget` form is the only working one. Verified in both directions inside the running container — the real path exits 0, a nonexistent path exits 1 — so the check can genuinely fail rather than passing unconditionally.
**Outcome:** applied
**Ref:** `docker-compose.yml`; `scripts/check-upstream-boundary.sh` `allowed_re`

## A19 — AC-05's "`lint:security` → 0 errors" was never achievable — gate-resolution

**Question:** Task 7 requires `npm run lint:security` to report 0 errors. It reports 48. Are these fork-introduced, and what does AC-05 become?
**Options considered:** re-derive AC-05 to fork-parity against an UPSTREAM*BASE control / additionally fix the `.vitepress/dist` eslint ignore gap (an upstream edit needing an allowance widening) / escalate and leave Task 7 blocked
**Chosen:** **Re-derive AC-05 to "no fork-introduced security errors", measured as parity against a control run at `UPSTREAM_BASE`.** The operator selected this option directly. No upstream file is touched.
**Decided-by:** human
**Justification:** Attribution was established by control run, not inference: a detached worktree at `597e3690` with `npm ci` and the \_identical* `lint:security` invocation reports **43 errors** — `no-unsanitized/property: 40`, `no-unsanitized/method: 3`. The fork, excluding gitignored build output, reports **43 with a byte-identical rule breakdown**, so the fork contributes **zero**. The criterion as written was therefore unmeetable from the moment the fork was taken, and satisfying it literally would mean editing ~25 upstream source files (`form-creator.ts`, `bookmark-pdf.ts`, `markdown-editor.ts`, …) to change `innerHTML` assignments — a change far outside the integration allowance, touching security-sensitive rendering paths the fork has no test coverage for, and guaranteeing merge conflicts against every future upstream pull. Parity is the property the fork can actually own and is what AC-05 was reaching for. **A known non-determinism is recorded rather than fixed:** `eslint.config.mjs` ignores `dist/**` but not `**/.vitepress/dist/**`, so a tree that has run a docs build reports **48** and a clean checkout reports **43** — the same source, two answers, depending on build history. It is left alone because `eslint.config.mjs` is upstream-owned and outside the allowance; the measurement protocol compensates by excluding `.vitepress/dist/` explicitly. Anyone re-measuring AC-05 must reproduce the control run rather than trusting a bare count, and must state whether build residue was present.
**Outcome:** applied
**Ref:** control worktree at `597e36904e8ccacc0af98463589805cb9ee910d2`; `eslint.config.mjs:11-21` (unmodified)

## A20 — `/tools/de/` under `vite preview` was a base-prefix bug in the locale middleware, not a preview-harness gap — gate-resolution

**Question:** The prior session recorded `/tools/de/` as "404s under `vite preview` while returning 200 in the Docker image — a preview-harness gap, not an app defect. Likely the real fix for test 1." Is that correct, and does AC-08 require serving the Docker image instead of preview?
**Options considered:** serve the `toolhub:local` image for e2e instead of preview / assert only against paths preview already maps / fix the base handling in the preview middleware
**Chosen:** **Fix the base handling.** `createLanguageMiddleware` strips `basePath` from the incoming path (`vite.config.ts:108-110`) and the 302 branch re-adds it (`:123`), but all 13 `req.url =` rewrites emit base-relative URLs that preview's static handler — which resolves against `base` — cannot find. A single `next` wrapper re-applies the prefix for the preview branch only.
**Decided-by:** agent
**Justification:** The recorded diagnosis was measured wrong in two ways, both traceable to a status-code-only probe. First, **without** `BASE_URL` at preview time, `/tools/de/` returns **200, not 404** — but serves the root `dist/index.html` (20153 bytes, `<html lang="en">`) instead of `dist/de/index.html` (19862 bytes, `<html lang="de" dir="ltr">`). A 200 carrying the wrong document is invisible to `curl -o /dev/null -w '%{http_code}'`, which is what the earlier session used; the "preview-harness gap" reading came from never comparing the body. Second, **with** `BASE_URL=/tools/` set at preview time (required because `vite.config.ts:540` reads `base` from it), the path genuinely 404s, and preview's own error body names the fix: "did you mean to visit /tools/de/index.html instead?" — i.e. the rewrite target must keep the prefix. Fixed at the single `next` choke point rather than at 13 assignment sites specifically because `vite.config.ts` is capped at 25 changed lines (A16); the change lands at exactly **25/25** and was tightened once rather than widening the cap. Scoped to `!isDev` because the dev branch is base-aware through Vite's transform pipeline and is not affected. Verified by byte-identity: `/tools/de/` now returns 19862 bytes matching `dist/de/index.html` exactly. Serving the image was rejected as it would couple every e2e run to a 439MB Docker build for a defect that is one prefix.
**Outcome:** applied
**Ref:** `vite.config.ts:104-121`; `e2e/i18n.spec.ts`; `playwright.config.ts`

## A21 — AC-08's ext-category assertion asserts a Phase 2 outcome and was marked `fixme`, not deleted — scope-boundary

**Question:** `e2e/i18n.spec.ts` test 1 asserts the German ext category "Office & Daten" is visible. It cannot pass. Delete it, weaken it, or register a tool to satisfy it?
**Options considered:** register a placeholder ext tool to make it green / delete the assertion / split it into a `test.fixme` with the precondition recorded
**Chosen:** **Split into `test.fixme`** with the precondition stated inline, and narrow test 1 to the upstream-category assertion it can actually make.
**Decided-by:** agent
**Justification:** Measured, not inferred: every entry in `extCategories` (`src/js/config/tools-ext.ts`) carries `tools: []` — the registry is scaffolding from Task 4 with zero tools registered — and `src/js/main.ts:313` filters empty categories out by design (A12). So the assertion is not failing; it is asserting a state Phase 1 does not produce. Registering a placeholder tool to turn it green was rejected outright: it would ship a fake tool to satisfy a test, and would make A12's filter — the thing under test — untestable. Deleting it was rejected because the assertion is correct and wanted, just premature; `test.fixme` keeps it in the file, visibly skipped in every run, so it resurfaces with the first ext tool instead of being silently lost. **A second wrong premise was corrected in the same file:** test 2 used `locale: 'fr-FR'` as "not in {en,de}", but `supportedLanguages` (`src/js/i18n/i18n.ts:8`) lists **21** locales including `fr`, and the build ships `dist/fr/index.html` — so the browser resolved French and never exercised the fallback at all. Changed to `fi-FI`, which misses on both the full tag and its primary subtag per `i18n.ts:90-97`. Before the routing fix this test failed with `lang="en"` (root fallback masking everything); after it, it failed with `lang="fr"` — the shape change is what exposed the bad premise.
**Outcome:** applied
**Ref:** `src/js/config/tools-ext.ts`; `src/js/main.ts:313`; `src/js/i18n/i18n.ts:8,90-97`

## A22 — `BACKLOG.md` is fork-owned and joins the allowance — gate-resolution

**Question:** The harvest User Story (US-TH-08) needs a backlog home. The allowance whitelists `DECISIONS.md` and `docs/specs/.*` but no backlog file, so a root `BACKLOG.md` fails `check-upstream-boundary.sh`. Widen the allowance, move the story into PosterEngine's backlog, or keep it in the spec only?
**Options considered:** widen `allowed_re` for `BACKLOG\.md` + record the widening / append US-TH-08 to `20_PosterEngine/BACKLOG.md#Ready` / fold the story into `docs/specs/` and have no backlog file
**Chosen:** **Widen the allowance** — `BACKLOG\.md` added to `allowed_re`, this entry records it.
**Decided-by:** human
**Justification:** `BACKLOG.md` is a new root file, not an upstream-owned one — upstream BentoPDF ships no backlog — so admitting it costs no rebase surface, which is what the allowance exists to bound. It follows the A9/A10/A15 precedent of widening by recorded decision rather than by silent edit. Moving the story to `20_PosterEngine/BACKLOG.md` was rejected because it would split toolhub's story record from its spec across two repos, and the boundary checker's own purpose is to keep fork-owned artefacts _in_ the fork. Spec-only was rejected because a story that exists in no backlog does not surface in `/kickoff` or a DOR sweep — it would be specced and then forgotten. **The gap was found by testing, not reading:** the checker iterates `git diff --name-only`, which lists tracked files only, so both new files passed while untracked; `git add -N` made them visible and the check then failed on `BACKLOG.md` as expected. An untracked new file is invisible to this gate — worth knowing for any future story that adds root files.
**Outcome:** applied
**Ref:** `scripts/check-upstream-boundary.sh:16`; `BACKLOG.md`; `docs/specs/2026-09-17-ext-tools-harvest-addendum.md`

## A23 — `localStorage` undefined under Node 26 is fixed by a fork-owned shim, not a test-env origin — gate-resolution

**Question:** 12 tests (`src/tests/i18n.test.ts` ×10, `src/tests/xss-replay.test.ts` ×2) died on `localStorage.clear()` with `TypeError: Cannot read properties of undefined`. Fix with `environmentOptions.jsdom.url`, a `NODE_OPTIONS` flag, a mock, or a shim?
**Options considered:** set `environmentOptions.jsdom.url` / `NODE_OPTIONS=--localstorage-file` / mock `localStorage` with `vi.fn()` / install a real spec-shaped Storage from a fork-owned setup module
**Chosen:** **A fork-owned shim**, `src/tests/x-localstorage-shim.ts`, called from `setup.ts`.
**Decided-by:** human
**Justification:** The cause is Node's, not jsdom's: Node >= 22 ships an experimental global `localStorage` that is `undefined` without `--localstorage-file` and occupies the global name before jsdom installs its own Storage — `window`/`document`/`navigator` all work while `localStorage` does not. Measured via a probe test, and confirmed by Node's own stderr (`ExperimentalWarning: localStorage is not available because --localstorage-file was not provided`) plus `node -e` with and without the flag. CI pins Node 20 (`.nvmrc`, workflows), where the global does not exist — so this bites only on a newer local Node. **The jsdom-origin fix was tried first and rejected on measurement:** jsdom does also refuse Storage on an opaque origin (proved directly: `SecurityError: localStorage is not available for opaque origins`), but that is a different failure and setting `url` fixed nothing here, because Node's global still shadows jsdom's. It also proved not origin-neutral — `url: 'https://localhost/'` made `digital-sign-pdf.ts:363`'s mixed-content guard (`pageIsHttps && tsaIsHttp`) fire for the first time and broke **6 previously-green** TSA tests, so changing the test-page origin has a blast radius across every protocol-dependent branch. `NODE_OPTIONS` worked (30/30) but makes a green suite depend on an env var and a stray DB file. A `vi.fn()` mock was refused outright: `xss-replay.test.ts` asserts the WasmProvider _removes_ poisoned entries from storage, so a stubbed seam would turn a security regression test into a false green — the shim therefore implements real deletion, and red/green was verified by bypassing the scrub in `wasm-provider.ts` and confirming the test fails. Suite: 924/936 → **936/936**.
**Outcome:** applied
**Ref:** `src/tests/x-localstorage-shim.ts`; `src/tests/setup.ts`; `vitest.config.ts`; `src/js/logic/digital-sign-pdf.ts:363`

## A24 — `eslint.config.mjs`, `vitest.config.ts` and `src/tests/setup.ts` join the capped tier — gate-resolution

**Question:** `npm run lint` failed with 1 error, in `docs/.vitepress/dist/assets/chunks/VPLocalSearchBox.*.js` — generated, untracked VitePress output linted as source. Fixing it and A23 touches three upstream-owned files outside the allowance. Widen how?
**Options considered:** add the three to the capped tier and move the shim body to a fork-owned `x-` file / add them and raise the cap 25 → 50 / trim the shim's comments to fit 25
**Chosen:** **Capped tier + shim extracted** to `src/tests/x-localstorage-shim.ts` (already fork-owned and uncapped via the `src/tests/x-.*` pattern).
**Decided-by:** human
**Justification:** The eslint ignore list already carried `**/.vitepress/cache/**` but not its sibling `**/.vitepress/dist/**`; the root `dist/**` glob is path-anchored in flat config and does not match a nested `dist`, so the docs bundle was linted as source. One added line fixes it. Raising the cap 25 → 50 was rejected because the cap exists to bound rebase conflict surface and the change would weaken it for every capped file to accommodate one. Trimming the comments was rejected because the rationale — why Node, not jsdom, and why the `jsdom.url` fix is wrong — is exactly the knowledge that cost two wrong turns to obtain; deleting it invites the retry. Extracting to an `x-` file keeps all of it in a fork-owned file the boundary checker does not cap, and leaves every upstream file far under the limit: `vitest.config.ts` 12/25, `src/tests/setup.ts` 8/25, `eslint.config.mjs` 1/25. Measured after the change: `npm run lint` exits **0** (was 1 error), warnings 2621 → 518 since the minified bundle is no longer scanned.
**Outcome:** applied
**Ref:** `eslint.config.mjs`; `scripts/check-upstream-boundary.sh` (capped list); `src/tests/x-localstorage-shim.ts`
